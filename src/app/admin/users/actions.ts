...
export async function backfillUserStatuses(): Promise<{ success: boolean; message: string; }> {
    try {
        ...
        for (const userDoc of usersSnapshot.docs) {
            ...
            const cashbackQuery = adminDb.collection('cashbackTransactions').where('userId', '==', userId).limit(1);
            const cashbackSnap = await cashbackQuery.get();

            if (!cashbackSnap.empty) {
                newStatus = 'Trader';
            } else {
                // Check if they are active
                const accountsQuery = adminDb.collection('tradingAccounts').where('userId', '==', userId).where('status', '==', 'Approved').limit(1);
...
export async function backfillUserLevels(): Promise<{ success: boolean; message: string; }> {
    try {
        ...
        const cashbackQuery = adminDb.collection('cashbackTransactions').where('date', '>=', monthStart);
        const cashbackSnap = await cashbackQuery.get();
